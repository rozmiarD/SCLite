...
def load_json_value(path: Path | str, *, error_cls: type[ErrorT], max_bytes: int | None = None) -> Any:
    json_path = Path(path)
    try:
        if max_bytes is not None and json_path.stat().st_size > max_bytes:
...
def load_json_object(path: Path | str, *, error_cls: type[ErrorT], max_bytes: int | None = None) -> Dict[str, Any]:
    value = load_json_value(path, error_cls=error_cls, max_bytes=max_bytes)
